    ...
    <h2>Blogger</h2>
        ...
        <div class="@if ($loop->index%2 != 0) blogger_even @else blogger_odd @endif">
            @if (!empty($autherData->image))
                <img src="{{ asset('images/profile/'. $autherData->image) }}" class="">
            @else
                <img src="{{ asset('images/profile/no_image/face_test.png') }}" class="">
            @endif
            <div class="blogger_name">{{ $autherData->name }}</div>
            ...
            <div class="to_blogger_category"><a href="{{ route('list.onlyAuther', ['auther_id' => $autherData->id]) }}">{{ $autherData->name }}の部屋へ</a></div>
        </div>
        @endforeach
    </section>
